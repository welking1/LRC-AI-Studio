...
- 可选接入本地 Whisper 自动对齐；不安装 AI 组件不影响基础使用
...
5. 点击「导出 LRC」。歌曲标题、歌手、专辑会自动写入 LRC 元数据。

已有 LRC 也可以直接导入，之后只需拖动播放位置或修改每行时间。
...
- LRC 是逐行时间标签格式；基础模式不伪造逐字时间。若需要逐字 KTV LRC，请使用可选 Whisper AI 对齐，并在导出前人工校准。
